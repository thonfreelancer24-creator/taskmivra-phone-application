# TaskMivra Crystal Voice — milestone 01
...
After reviewing and configuring a real model as described in `docs/MODELS.md`:
...
```bash
crystal-voice ui --adapter spexplus --port 8765
...
**No candidate is accepted yet.** Network policy prevented fetching the two upstream repositories/checkpoints, and the repository contains no private real-speaker fixtures. The generated diagnostic benchmark tests infrastructure only. Mandatory music, TV, competing-speaker, fan/AC, keyboard, mixed-challenge listening and independent speaker-embedding/STOI/PESQ measurement remain undone. See `docs/QUALITY_GATES.md` for the exact distinction between machine rejection and human acceptance.
