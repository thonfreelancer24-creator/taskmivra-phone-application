...
def main() -> None:
    ...
    # The released Hugging Face repository stores the training configuration as
    # config.yaml. Crystal Voice keeps a descriptive local filename so the model
    # family/sample-rate contract is explicit to operators and provenance tools.
    assets = (
        ("config.yaml", "config_wsj0-2mix_speech_SpEx-plus_2spk.yaml"),
        ("last_best_checkpoint.pt", "last_best_checkpoint.pt"),
    )
    records = []
    for repository_name, local_name in assets:
        source = unique(snapshot, repository_name)
        destination = args.destination / local_name
        shutil.copyfile(source, destination)
        records.append({
            "filename": local_name,
            "repository_filename": repository_name,
